...
const PRONUNCIATION_METRICS = {
  ...
  { key: 'stress', label: 'Accuracy Score', value: 85, verdict: 'Natural emphasis' },
  { key: 'intonation', label: 'Prosody Score', value: 82, verdict: 'Mostly clear melody' },
  { key: 'phoneme', label: 'Fluency Score', value: 82, verdict: 'A few sound refinements' }]
...
const KEY_VOCAB = [
  ...
  { w: 'actually', t: 'freq' }, { w: 'basically', t: 'freq' },
  { w: 'important', t: 'basic' }, { w: 'think', t: 'basic' }, { w: 'good', t: 'basic' }, { w: 'make', t: 'basic' },
];
...
const AI_TIPS = [
  { title: 'Focus on Articles (the / a / an)', body: 'You repeatedly omit "the" before specific places (the hospital, the office). Review definite article rules before your next session — they signal shared knowledge between speaker and listener.' },
  { title: 'Try Complex Sentence Structures', body: 'Boost your speaking depth by using concessive clauses: "Although…", "Even though…", or "While…" to connect ideas — this will push your score into the top band.' },
  { title: 'Build on Your Strong Vocabulary', body: 'You used advanced words like "sustainable" and "nevertheless" naturally. Try linking them into longer arguments to show command, not just recall.' },
];
      ...
      <div className="pr-body">
          ...
          <div className="pr-subs">
            <span className={`sub-pill ${bandClass(stress)}`}>
              <span className="label">Accuracy Score</span><span className="v">{stress}</span>
            </span>
            <span className={`sub-pill ${bandClass(intonation)}`}>
              <span className="label">Prosody Score</span><span className="v">{intonation}</span>
            </span>
            <span className={`sub-pill ${bandClass(phoneme)}`}>
              <span className="label">Fluency Score</span><span className="v">{phoneme}</span>
            </span>
          </div>
        </div>
      }
...
function GrammarErrorSection() {
  return (
    <div>
      <div className="panel-sub">
        <span className="t"><span className="lbl">Grammar Error Analysis</span></span>
        <span className="meta">{GRAMMAR_ERRORS.length} TYPES</span>
      </div>
      <div className="err-analysis-card">
        <div className="err-analysis-rows">
          {GRAMMAR_ERRORS.map((e, i) =>
            <div key={i} className="err-analysis-row">
              <span className={`err-type-tag ${e.tagCls}`}>{e.tag}</span>
              ...
              <span className="err-count">×{e.cnt}</span>
            </div>
          )}
        </div>
      </div>
...
function VocabSection() {
  return (
    <div>
      <div className="panel-sub">
        <span className="t"><span className="lbl">Key Vocabulary Used</span></span>
        <span className="meta">{KEY_VOCAB.length} WORDS</span>
      </div>
      <div className="vocab-card">
        ...
        <div className="vocab-legend">
          <span className="vleg-item"><span className="vleg-dot new"></span>New</span>
          <span className="vleg-item"><span className="vleg-dot adv"></span>Advanced</span>
          <span className="vleg-item"><span className="vleg-dot freq"></span>Frequent</span>
          <span className="vleg-item"><span className="vleg-dot basic"></span>Basic</span>
        </div>
      </div>
    </div>);
}
...
function AITipsSection() {
  return (
    <div>
      <div className="panel-sub">
        <span className="t">
          <span className="lbl">AI-Personalized Learning Tips</span>
        </span>
        <span className="meta">{AI_TIPS.length} TIPS</span>
      </div>
