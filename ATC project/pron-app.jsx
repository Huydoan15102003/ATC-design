      ...
      <div className="picker" onClick={() => setOpen((v) => !v)}>
        {selected ? (
          ...
          <>
            ...
            <span style={{ color: 'var(--muted)' }}>Assign to student…</span>
          </>
        )}
...
function SetupPane({ sentences, setSentences, student, setStudent, currentIdx, doneCount, canStart, onStart, phase }) {
...
function App() {
  ...
  const filledCount = sentences.filter((s) => s.trim()).length;
  const canStart = filledCount >= 3 && !!student;
